...
gs = gridspec.GridSpec(1,3,bottom=.37)
...
for evol_env in evolution_envs:
	...
	legend_handle3 = axa.errorbar(x_scale_list+.005*env_ind, numpy.var(nacl_fit_dict[evol_env],axis=0), yerr=numpy.var(nacl_fit_dict[evol_env],axis=0)*numpy.sqrt(2)/numpy.sqrt(clone_count - 1), marker='o', color = color_list[env_ind], linewidth = 3, elinewidth = 2)
	legend_handles_list.append(legend_handle3)
	env_ind += 1
...
axa.set_xticklabels( ['0','.2', '.4', '.8'], fontsize=12 )
...
axa.legend( legend_handles_list, legend_label_list, numpoints = 1, loc='upper left', fontsize=8)
...
axa.set_ylabel( 'Variance in relative fitness (%)' )
...
for evol_env in evolution_envs:
	...
	legend_handle = axb.errorbar(x_scale_list+.02*env_ind, numpy.var(ph_fit_dict[evol_env],axis=0), yerr=numpy.var(ph_fit_dict[evol_env],axis=0)*numpy.sqrt(2)/numpy.sqrt(clone_count - 1), marker='o', color = color_list[env_ind], linewidth = 3, elinewidth = 2)
	#axb.plot(x_scale_list[env_ind]+.02*env_ind, numpy.std(ph_fit_dict[evol_env],axis=0)[env_ind], color = color_list[env_ind], marker='o', markersize=8)
...
axb.legend( legend_handles_list, legend_label_list, numpoints = 1, fontsize=8)
#axb.set_ylim([-6,8])
axb.set_xlim([3 - 4.3*pad, 7.3 + 4.3*pad])
axb.set_xticks(x_scale_list)
axb.set_xticklabels( ['3','3.8', '4.5', '6', '7.3'], fontsize=12 )
...
for evol_env in evolution_envs:
	...
	legend_handle = axc.errorbar(x_scale_list+.1*env_ind, numpy.var(temp_fit_dict[evol_env],axis=0), yerr=numpy.var(temp_fit_dict[evol_env],axis=0)*numpy.sqrt(2)/numpy.sqrt(clone_count - 1), marker='o', color = color_list[env_ind], linewidth = 3, elinewidth = 2)
	#if env_ind < 1.5:
...
axc.legend( legend_handles_list, legend_label_list, numpoints = 1, loc='upper left', fontsize=8)

axc.set_xlim([ 21 - 16*pad, 37 + 16*pad])
axc.set_xticks( x_scale_list)
axc.set_xticklabels( ['21','30', '34','37'], fontsize=12 )
...
pt.savefig('figures/S11_gradient_variance_draft.pdf',bbox_inches='tight')
pt.close()
